...
<style>
:root{--teal:#085041;--coral:#712B13;--paper:#FBFAF7;--gold:#B8860B;
--ink:#1d2a26;--line:#d8d3c6}
*{box-sizing:border-box}
body{margin:0;background:var(--paper);color:var(--ink);
font:15px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif}
.wrap{max-width:980px;margin:0 auto;padding:20px 16px 28px}
.eyebrow{font-size:11px;letter-spacing:.14em;color:var(--gold);
text-transform:uppercase;font-weight:700}
h1{font-family:Georgia,'Times New Roman',serif;color:var(--teal);
font-size:clamp(19px,3.4vw,27px);margin:.2em 0 .1em;line-height:1.2}
.sub{color:#5b6763;font-size:13px;margin-bottom:14px}
...
.ctrl label{font-size:11px;font-weight:700;letter-spacing:.06em;
text-transform:uppercase;color:var(--teal)}
select{appearance:none;-webkit-appearance:none;background:#fff;color:var(--ink);
border:1.5px solid var(--teal);border-radius:6px;padding:8px 30px 8px 10px;
font-size:14px;width:100%;background-image:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='10' height='6'><path d='M0 0l5 6 5-6z' fill='%23085041'/></svg>");
background-repeat:no-repeat;background-position:right 10px center}
select:focus{outline:3px solid rgba(8,80,65,.25)}
#sexWrap.hidden{display:none}
.chartbox{background:#fff;border:1px solid var(--line);border-radius:10px;
padding:10px 8px 4px;position:relative}
svg text{font:11px -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;
fill:#5b6763}
.tip{position:absolute;pointer-events:none;background:var(--teal);color:#fff;
...
background:var(--teal);color:var(--paper);padding:2px 8px;border-radius:5px;
...
function draw(){
 ...
 if(e.s==="S"){
  ...
  g+='<path d="'+path(e.v)+'" fill="none" stroke="#085041" stroke-width="1.1" opacity="0.33"/>';
  g+='<path d="'+path(ma12(e.v))+'" fill="none" stroke="#085041" stroke-width="2.4"/>';
 }
